...
use corvus_core::{FingerprintEvent, PcapFileSource, Pipeline, PipelineConfig, SourceError};

use crate::live::{DEFAULT_BPF_FILTER, LiveConfig, LiveSource};
use crate::report::ReportBuilder;
use crate::serve::{self, ServeConfig, Source};
use corvus_intel::{Alert, FpKind, IntelStore, MatchReport, MatchStrength, default_db_path};
...
pub enum Command {
    ...
    /// target/debug/corvus or target/release/corvus when running from cargo.
    ///
    ...
    ///
    ///   corvus live eth0 --filter "tcp port 443"
    ///
    ...
    /// process, the same as `corvus live` but rendered in the browser. With
    /// neither, the stream tails the database, so a separate
    /// `corvus live --detect` writing to the same store surfaces here.
    Serve {
...
/// The subcommands under `corvus intel`.
#[derive(Debug, Subcommand)]
...
fn open_for_report(db: Option<&Path>) -> Result<Option<IntelStore>> {
    ...
    tracing::info!(
        path = %path.display(),
        "no intelligence database found; reporting fingerprints only, run 'corvus intel seed' to add verdicts"
    );
    Ok(None)
}
...
fn open_for_run(intel: bool, detect: bool, db: Option<&Path>) -> Result<Option<IntelStore>> {
    ...
    if !path.exists() {
        if detect {
            tracing::warn!(
                path = %path.display(),
                "no intelligence database found; creating one to record detections, run 'corvus intel seed' to enable known-bad matching"
            );
            return Ok(Some(open_or_create(&path)?));
        }
        tracing::warn!(
            path = %path.display(),
            "no intelligence database found; run 'corvus intel seed' first, continuing without it"
        );
        return Ok(None);
    }
