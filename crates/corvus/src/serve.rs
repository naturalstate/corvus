...
use corvus_core::{FingerprintEvent, PcapFileSource, Pipeline, PipelineConfig};
use corvus_intel::{Alert, CatalogEntry, FpKind, IntelStore, MatchReport, Rule, Stats};
...
pub enum Source {
    ...
    /// `corvus live --detect` writes to the same store.
    Tail,
    ...
    if csv {
        ...
        Ok((
            [
                ...
                (
                    ...
                    "attachment; filename=\"corvus-alerts.csv\"",
                ),
            ],
            body,
        )
        ...
        Ok((
            [
                ...
                (
                    header::CONTENT_DISPOSITION,
                    "attachment; filename=\"corvus-alerts.json\"",
                ),
            ],
            body,
        )
...
/// each new one. Used when nothing is captured in process: a `corvus live
